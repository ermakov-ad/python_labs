...
T = 5   # период колебаний
#A = 0.5   # амплитуда колебаний
# добавим затухание
def A(t):
    return 0.5*math.exp(-t/3/T)
# угловая скорость
def omega(t):
    return A(t)*math.cos(2*math.pi*t/T)
...
class CalcMesh:
    ...
    def vel(self, t):
        self.velocity[1] = -omega(t)*self.nodes[2]
        self.velocity[2] = omega(t)*self.nodes[1]
    ...
    # отдельный метод для вращения относительно оси x без деформации
    def move_round(self, tau, t):
        # пересчитываем скорости
        self.vel(t)
    ...
    def snapshot(self, snap_number):
        ...
        writer.SetFileName("cos_oscillation_ship_with_resistance_" + str(snap_number) + ".vtu")
        writer.Write()
...
try:
    ...
    gmsh.merge(os.path.join(path, 'ship.stl'))
except:
