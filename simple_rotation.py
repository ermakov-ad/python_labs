...
class CalcMesh:
    ...
    # отдельный метод для вращения без деформации
    def move_round(self, tau, t):
        # пересчитываем скорости
        self.vel(t)
        r0 = np.sum(np.multiply(self.nodes, self.nodes))
        self.nodes += self.velocity * tau
        r = np.sum(np.multiply(self.nodes, self.nodes))
        self.nodes = np.dot(math.sqrt(r0/r), self.nodes)
    ...
    def snapshot(self, snap_number):
        ...
        writer.SetFileName("rotation_norm_t=0.01_" + str(snap_number) + ".vtu")
        writer.Write()
...
try:
    ...
    gmsh.merge(os.path.join(path, 'arrow.stl'))
except:
